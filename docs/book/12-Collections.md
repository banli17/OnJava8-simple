...
通常，对象的数量和类型是根据运行时数据(比如 for 循环)来创建的。因此，不能依靠单一的引用来持有每一个对象：
...
大多数编程语言都提供了方法来解决这个问题。Java 有多种方式保存对象。例如数组，它是编译器支持的类型。数组是保存一组对象的最有效的方式，如果想要保存一组基本类型数据，也推荐使用数组。但是数组具有固定的长度，而写程序的时候并不知道需要多少个对象，因此数组就太受限了。

**java.util** 库提供了一套*集合类*（collection classes）来解决这个问题，其中基本的类型有 **List** 、 **Set** 、 **Queue** 和 **Map**。这些类型也被称作*容器类*（container classes）。

集合还有一些其它特性。例如， **Set** 对于每个值都只保存一个对象， **Map** 是一个关联数组，允许将某些对象与其他对象关联起来。Java 集合类都可以自动地调整大小。因此，在编程时，可以将任意数量的对象放置在集合中。

在本章中，将介绍 Java 集合类库的基本知识。在[附录：集合主题]()中，还将学习到其余的那些集合和相关功能。

<!-- Generics and Type-Safe Collections -->

## 泛型和类型安全的集合

使用 Java 5 之前的集合的一个主要问题是编译器允许你向集合中插入不正确的类型。例如，考虑一个 **Apple** 对象的集合，这里使用 **ArrayList** 来存储，它可以看作是“自动扩容的数组”。

使用 **ArrayList** 相当简单：创建一个实例后。

- `add()` : 插入对象。
- `get(index)`: 获取对象，index 表示索引，就像数组那样，但是不需要方括号。
- `size()` : 获取集合中包含多少个元素。
...
**Apple** 和 **Orange** 是不同的，虽然它们除了都是 **Object** 对象。因为 **ArrayList** 保存的是 **Object** ，所以可以通过 `add()` 方法放入 **Apple** 对象，同时也可以放入 **Orange** 对象，这在编译期和运行时都不会有问题。当使用 `get()` 方法来取出 **Apple** 对象时，得到的只是 **Object** 引用，必须将其转型为 **Apple**。否则，将会产生语法错误。

在运行时，当尝试将 **Orange** 对象转为 **Apple** 时，会出现输出中显示的错误。

使用 Java 泛型来创建类可能很复杂。但是，使用预先定义的泛型类却相当简单。例如，要定义一个用于保存 **Apple** 对象的 **ArrayList** ，只需要使用 **ArrayList\<Apple\>** 来代替 **ArrayList** 。`\<Apple\>`是*类型参数*（可以多个），它指定了这个集合可以保存的类型。

通过使用泛型，就可以在编译期防止将其他类型对象放置到集合中。下面还是这个示例，但是使用了泛型：
...
在 **apples** 定义的右侧，可以看到 `new ArrayList<>()` 。这称为“菱形语法”（diamond syntax）。在 Java 7 之前，必须在两端都进行类型声明，如下所示：

```java
ArrayList<Apple> apples = new ArrayList<Apple>();
```

_类型推断_（type inference）可以直接从左侧获得，没必要两端都声明，所以，Java 团队进行了改进。

指定类型后，编译器会阻止将 **Orange** 放入 **apples** ，因此，它会产生一个编译时错误，而不是运行时。

使用泛型，从 **List** 中获取元素不需要强制类型转换。因为 **List** 知道它持有什么类型，因此当调用 `get()` 时，它会替你执行转型。因此，使用泛型，编译器将检查放入集合的对象类型，而且在使用集合对象时也更清晰。
...
Java 1.0 中的 **Stack** 类设计得很糟糕。Java 6 添加了 **ArrayDeque** ，其中包含直接实现堆栈功能的方法：
...
**Set** 不保存重复的元素。 如果试图将相同对象的多个实例添加到 **Set** 中，那么它会阻止这种重复行为。 **Set** 最常见的用途是查找某个对象是否在一个 **Set** 中。因此，通常会选择 **HashSet** 实现，该实现针对快速查找进行了优化。
